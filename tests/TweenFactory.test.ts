...
describe('TweenFactory', () => {
	...
	it('should throw for invalid configurations', () => {
		...
		}, 'settings be valid').toThrow();
		expect(() => {
			tweener({easing: 'hello'} as any);
		}, 'settings.easing should be a function').toThrow();
		expect(() => {
			tweener({easing: () => 'hello'} as any);
		}, 'settings.easing function should return a number').toThrow();
		expect(() => {
			tweener({duration: 'hello'} as any);
		}, 'settings.duration should be a number').toThrow();
		expect(() => {
			tweener(NaN);
		}, 'duration cannot be NaN').toThrow();
		expect(() => {
			tweener({duration: NaN} as any);
		}, 'settings.duration cannot be NaN').toThrow();
		expect(() => {
			tweener(-1);
		}, 'duration should be a positive value').toThrow();
		expect(() => {
			tweener({duration: -1} as any);
		}, 'settings.duration should be a positive value').toThrow();
		expect(() => {
			tweener(Infinity);
		}, 'duration must be a finite value').toThrow();
		expect(() => {
			tweener({duration: Infinity} as any);
		}, 'settings.duration must be a finite value').toThrow();
	...
	it('should react to manually calling complete', () => {
		...
		const a = tween.start();
		expect(state.started, 'started').toBe(true);
		a.complete();
		expect(state.completed, 'completed').toBe(true);
	...
	it('should react to manually calling dispose', () => {
		...
		a.dispose();
		expect(state.completed, 'completed').toBe(false);
		expect(state.disposed, 'disposed').equal(-1);
...
async function test (tweenFactory: tweening.FactoryBuilder)
{
	...
	expect(() => { tween1.add({x: 0}, {x: 10}); }).toThrow();
	expect(() => { tween1.chain(); }).toThrow();
	expect(state1.started, 'state1.started').toBe(true);
	expect(isNaN(a.lastUpdate)).toBe(true);
	await delay(10);
	expect(a.timeFrame.progress).not.toBe(0);
	await delay(100);
	expect(isNaN(a.lastUpdate)).toBe(true);
	expect(a.timeFrame.range.start, 'timeFrame.range.start').toBeGreaterThan(0);
	...
	tweenFactory.active.update();
	expect(isNaN(a.lastUpdate), 'lastUpdate').not.toBe(true);
	expect(a.timeFrame.progress, 'progress').equal(1);
	expect(state1.completed, 'state1.completed').toBe(true);
	expect(state1.disposed, 'state1.disposed').equal(1);

	expect(state2.started, 'state2.started').toBe(true);
	await delay(110);
	tweenFactory.active.update();
	expect(state2.completed, 'state2.completed').toBe(true);
